...
# 		# 유효한 인덱스 찾기
# 		valid_idx = (self._reward != 0.0).nonzero(as_tuple=False).squeeze()

# 		for t in range(self.cfg.horizon+1):
# 			_idxs = idxs + t
# 			invalid_mask = self._reward[_idxs] == 0.0
# 			if invalid_mask.any():
# 				valid_indices = valid_idx.unsqueeze(0).expand(invalid_mask.sum(), -1)
# 				invalid_indices = _idxs[invalid_mask].unsqueeze(1)
# 				next_valid_indices = valid_indices > invalid_indices
# 				next_valid_indices = next_valid_indices.to(dtype=torch.int)
# 				_idxs[invalid_mask] = valid_idx[next_valid_indices.argmax(dim=1)]
